...
install_emscripten: build_libs
	@$(PERL) $(TOP)/util/mkdir-p.pl $(EMINSTALLDIRS)
	@set -e; for i in include/openssl/*.h; do \
	(cp $$i $(INSTALL_PREFIX)$(INSTALLTOP)/$$i; \
	chmod 644 $(INSTALL_PREFIX)$(INSTALLTOP)/$$i ); \
	done;
	cp libcrypto.a $(INSTALL_PREFIX)$(INSTALLTOP)/$(LIBDIR)
	cp libssl.a $(INSTALL_PREFIX)$(INSTALLTOP)/$(LIBDIR)
